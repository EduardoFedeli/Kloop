...
export function BottomNav({ unreadCount, isLoggedIn = false }: Props) {
  ...
  return (
    <>
      <nav className={cn(
        "md:hidden fixed bottom-0 left-0 right-0 z-40 border-t safe-area-inset-bottom",
        isComunidades 
          ? "bg-eclipse border-void-navy" 
          : "bg-white dark:bg-[var(--color-pine)] border-gray-100 dark:border-[var(--color-forest)]"
      )}>
              ...
              <li key={href} className="flex-1">
                <Link
                  ...
                  className={cn(
                    ...
                    isActive
                      ? (isComunidades ? "text-neon-lift" : "text-[var(--color-teal)] dark:text-[var(--color-celadon)]")
                      : (isComunidades ? "text-neon-mist/60 hover:text-neon-lift" : "text-gray-400 dark:text-[var(--color-sage)] hover:text-[var(--color-teal)] dark:hover:text-[var(--color-celadon)]"),
                  )}
                >
                  <div className={cn("relative", isAction && (isComunidades ? "text-neon-lift" : "text-[var(--color-teal)] dark:text-[var(--color-celadon)]"))}>
                    <Icon
                      size={isAction ? 28 : 22} // Aumenta o tamanho do botão central
                      strokeWidth={isActive || isAction ? 2.5 : 1.8}
                      className={isActive ? (isComunidades ? "fill-neon-lift/10" : "fill-[var(--color-teal)]/10 dark:fill-[var(--color-celadon)]/10") : ""}
                    />
