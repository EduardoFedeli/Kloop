...
const STATUS_LABEL: Record<OfferStatus, string> = {
  PENDING_SELLER: 'aguardando o vendedor',
  PENDING_BUYER: 'aguardando sua resposta',
  ACCEPTED: 'aceita — pague em até 24h',
  REJECTED: 'recusada',
  EXPIRED: 'expirada',
  CANCELLED: 'cancelada',
}
...
function OfferCard({ offer }: { offer: OfferRow }) {
  const isUrgent = offer.status === 'PENDING_BUYER' || offer.status === 'ACCEPTED'
...
export default async function ComprasOfertasPage() {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 px-8 text-center gap-5">
        <div className="w-16 h-16 rounded-full bg-gray-100 dark:bg-white/5 flex items-center justify-center">
          <Handshake size={28} className="text-gray-300 dark:text-sage/30" />
        </div>
        <div className="space-y-2">
          <h3 className="text-[17px] font-black text-[var(--foreground)] tracking-tight">nenhuma oferta feita</h3>
          <p className="text-[13px] text-gray-500 dark:text-sage leading-relaxed max-w-[280px]">
            quando você fizer uma oferta em algum anúncio, ela aparece aqui com o andamento.
          </p>
        </div>
        <Link
          href="/"
  ...
  return (
    <div className="space-y-6">
      <Section title="aguardando sua resposta" offers={pendingBuyer} />
      <Section title="aceitas — pague em até 24h" offers={accepted} />
      <Section title="aguardando o vendedor" offers={pendingSeller} />
